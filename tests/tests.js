...
describe('common usage', function() {
	it('works correctly for common operations', function() {
		...
		assert.equal(
			jsce('foo</script>bar</style>baz</script>qux', {
				'isScriptContext': true
			}),
		...
		assert.equal(
			jsce({ 'foo\x00bar\uFFFDbaz': 'foo\x00bar\uFFFDbaz' }),
